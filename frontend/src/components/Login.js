...
import {
  Box, Typography, TextField, Button, CssBaseline, Avatar,
  Container, Paper, AppBar, Toolbar, useMediaQuery
} from '@mui/material';
...
const Login = ({ setAuth }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const response = await axios.post('http://192.168.1.15:5000/api/auth/login', {
      ...
      const errorMessage = error.response?.data?.error ||
        (error.response?.status === 500 ? 'Error en el servidor' : 'Credenciales incorrectas');
  ...
  return (
    <ThemeProvider theme={theme}>
      <CssBaseline />
      <AppBar position="fixed" sx={{ backgroundColor: '#c0c0c0', boxShadow: 'none' }}>
        <Toolbar>
          <Box
            component="img"
