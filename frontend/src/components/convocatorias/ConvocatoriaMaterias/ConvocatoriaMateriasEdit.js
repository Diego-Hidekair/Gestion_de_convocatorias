...
import { 
  ...
  IconButton, Paper, Box, Alert, Divider, Chip, Tooltip, CircularProgress 
} from '@mui/material';
...
const ConvocatoriaMateriasEdit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Cargar materias ya asignadas a la convocatoria, organizadas por ítem
        try {
          const asignadasResponse = await api.get(`/convocatoria-materias/${id_convocatoria}/materias`);
          if (asignadasResponse.data && Object.keys(asignadasResponse.data).length > 0) {
            const nuevosItems = [];
            Object.keys(asignadasResponse.data).forEach(itemKey => {
              nuevosItems.push({
                item: parseInt(itemKey),
                materias: asignadasResponse.data[itemKey].map(m => ({
                  ...m,
                  total_horas: (m.horas_teoria || 0) + (m.horas_practica || 0) + (m.horas_laboratorio || 0)
                }))
              });
            });
  ...
  const calcularHorasTotales = (materias) => materias.reduce((sum, m) => sum + (m.total_horas || 0), 0);
  const calcularHorasGlobales = () => items.reduce((acc, it) => acc + calcularHorasTotales(it.materias), 0);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const datosParaEnviar = {
        items: items.map(it => ({
          item: it.item,
          materias: it.materias.map(m => ({ id_materia: m.id_materia }))
        }))
      };
      await api.put(`/convocatoria-materias/${id_convocatoria}/materias`, datosParaEnviar);
  ...
  const handleVolver = () => navigate(`/convocatorias/editar/${id_convocatoria}`);
  ...
  if (!convocatoriaData) {
    return (
      <Container maxWidth="md" sx={{ mt: 4 }}>
        <Alert severity="error">{error || 'No se pudo cargar la información de la convocatoria'}</Alert>
      ...
      <Paper elevation={3} sx={{ p: 3, mt: 4 }}>
        ...
        {convocatoriaData && (
          <Box sx={{ mb: 3, p: 2, backgroundColor: '#f5f5f5', borderRadius: 1 }}>
            <Typography variant="subtitle1"><strong>Convocatoria:</strong> {convocatoriaData.nombre_conv}</Typography>
            <Typography variant="body2"><strong>Programa:</strong> {convocatoriaData.programa || convocatoriaData.nombre_programa}</Typography>
            <Typography variant="body2"><strong>Tipo:</strong> {convocatoriaData.nombre_tipoconvocatoria || convocatoriaData.nombre_tipo_conv}</Typography>
          </Box>
        )}
          ...
          return (
            <Box key={itemIndex} sx={{ mb: 3, p: 2, border: '1px solid #ddd', borderRadius: 2 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                ...
                {items.length > 1 && <Button color="error" onClick={() => handleRemoveItem(itemIndex)}>Eliminar Ítem</Button>}
              </Box>
                ...
                <FormControl fullWidth>
                  ...
                  >
                    ...
                    {materias.map(materia => (
                      ...
                      >
                        {materia.materia} ({materia.cod_materia}) - Teoría: {materia.horas_teoria}h, Práctica: {materia.horas_practica}h, Lab: {materia.horas_laboratorio}h
                      </MenuItem>
                    ))}
                  </Select>
                </FormControl>
              ...
              {it.materias.length > 0 && (
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 2 }}>
                  <Chip label={`Horas totales ítem ${it.item}: ${totalHorasItem}`} color="primary" sx={{ fontWeight: 'bold' }} />
                </Box>
              )}
            </Box>
          );
