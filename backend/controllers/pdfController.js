...
exports.generatePDF = async (req, res) => {
    ...
    try {
        ...
        console.log('Ruta de la carpeta PDF:', pdfDirectory); // <-- Verifica si está mostrando la ruta correcta
    console.log('Ruta completa del PDF:', pdfPath); // <-- Verifica si el nombre del archivo es correcto


        // Asegurar que el directorio de PDFs existe
        if (!fs.existsSync(pdfDirectory)) {
            console.log('Directorio de PDFs no existe. Creando...'); // <-- Asegúrate de que esto no sea un problema
        ...
        doc.end();
        console.log('PDF generado correctamente:', pdfPath); // <-- Verifica si el PDF se genera con éxito
        const filePath = path.join(__dirname, 'pdfs', `convocatoria_${id}.pdf`);
        console.log("Guardando PDF en la ruta:", filePath);
